...
Together they provide the AppID, title, platforms, Metacritic score, localized description, release date, developers, publishers, genres, artwork, and screenshots. Imported cards use the conventional vertical library asset when it exists and fall back to Store artwork otherwise. Detail enrichment accepts named demos and mods already present in the owned library. An unavailable Store entry preserves its ownership title and tries verified vertical or legacy header CDN artwork, but remains pending when Steam cannot provide the description and managed cover required for a usable card.
...
The owned-library call runs only after the user selects **Refrescar ahora**. `GetOwnedGames` is the sole ownership source. GameVault saves that complete snapshot before sending each new or pending AppID through the same Store catalog adapter used by manual addition. Metadata completion also covers retained entries absent from a later ownership snapshot because their canonical game remains in the local library. A provider entry is marked complete only after its merged card has a title, description, and managed local cover. Pending metadata resumes at application startup and after a Store `Retry-After` delay without refreshing ownership. Other failures remain pending for the next startup rather than entering an unbounded retry loop. Progress is visible throughout the application and can be cancelled without discarding completed cards. When Store returns metadata whose embedded `steam_appid` differs from the requested AppID, GameVault treats it as an alias. If Store metadata would give distinct AppIDs the same visible title, GameVault retains the unique ownership title. Explicit catalog-field edits and all personal fields are preserved. Playtime and unrelated fields are ignored. There is no scheduled, background, or realtime ownership synchronization.
...
The add-game dialog presents category-specific Spanish guidance and keeps the current query available for an explicit retry. Steam authentication failures require reconnecting its web session or replacing its advanced key. RAWG authentication failures offer actions to replace or remove keys stored by GameVault, or direct environment-key users to `RAWG_API_KEY`. Steam metadata enrichment stops the current pass on a rate limit and resumes after `Retry-After`, or after a short fallback delay when Steam omits it. Offline and timeout failures stop the pass and remain pending for startup; other catalog operations do not retry automatically.

Catalog access is optional. A failed request does not change SQLite data or prevent browsing, editing, or manually extending the local library. Previously imported text remains available offline. Steam covers and backgrounds are downloaded into managed local storage; screenshots and RAWG artwork remain remote and require a connection.
